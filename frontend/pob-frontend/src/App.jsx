...
function App() {
  ...
  const [error, setError] = useState(''); // Tracking login errors
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // If successful, we capture the JWT!
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          {error && (
            <div className="bg-red-500/10 border border-red-500/50 text-red-500 text-xs p-3 rounded-lg text-center font-medium">
              {error}
            </div>
          )}
          ...
          <button 
            type="submit"
            className="w-full bg-blue-600 hover:bg-blue-500 text-white font-semibold py-3 rounded-lg transition-all active:scale-[0.98] shadow-lg shadow-blue-900/20"
          >
            Sign In
          </button>
        </form>
